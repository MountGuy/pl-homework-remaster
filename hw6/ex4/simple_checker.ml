...
let rec solve_equ equs =
  ...
  (
    ...
    | (TFun(t1f, t1s), TFun(t2f, t2s)) -> solve_equ ([(t1f, t2f); (t1s, t2s)] @ equ_tail)
...
let check : M.exp -> M.types = fun exp ->
  ...
  let rec get_answer t sols =
    match sols with
    | [] -> t
    | (v, v_t) :: sol_tail -> get_answer (app_sol v_t v t) sol_tail in
  let t' = get_answer t sols in
  let rec tvar_to_mtyp t =
    match t with
    | TInt -> M.TyInt
    | TBool -> M.TyBool
    | TString -> M.TyString
    | TPair (t1, t2) -> M.TyPair(tvar_to_mtyp t1, tvar_to_mtyp t2)
    | TLoc (t) -> M.TyLoc(tvar_to_mtyp t)
    | TFun (t1, t2) -> M.TyArrow(tvar_to_mtyp t1, tvar_to_mtyp t2)
